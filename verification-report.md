...
| 8 | Persists through the actor state store; store encryption therefore encrypts history | A §4 | **CONFIRMED** | Same architecture page; [`dapr/pkg/runtime/wfengine/backends/actors/`](https://github.com/dapr/dapr/tree/master/pkg/runtime/wfengine/backends/actors) — the only backend directory is `actors` | Docs: state persisted in "the configured actor state store". Encryption is applied by `TryEncryptValue` at the state-store layer, so it covers the same bytes. Sound. |
